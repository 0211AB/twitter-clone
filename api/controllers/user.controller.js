...
export const register = async (req, res) => {
    ...
    if (!username || !password || !mail || !name) return res.status(400).send("Bad fields")
    const createUID = uid()
    ...
    const createdUser = await userSchema.find({ $or: conditions })
    if (createdUser.length > 0) return res.status(500).send("Username/Email is already taken")
    await newUser.save();
...
export const getUser = async (req, res) => {
    ...
    if (!id && !username) return res.status(406).send("Incorrect Data")
...
export const getUserTweets = async (req, res) => {
    ...
    if (!user) return res.status(406).send("Incorrect Data")
...
export const createTweet = async (req, res) => {
    ...
    if (!user || !date || !content) return res.status(500).send("Error")
    if (content.length < 3) return res.status(500).send("Please Enter Conent with length > 3")
...
export const updateUser = async (req, res) => {
    const { id, description, name, photo, banner } = req.body

    if (!id || name?.length < 3) return res.status(500).send("Error")
...
export const followManager = async (req, res) => {
    ...
    if (!followTo) return res.status(500).send("Error")
...
export const uploadImages = async (req, res) => {
    ...
    for (let i = 0; i < req.files.length; i++) {
        ...
        const imgPath = `http://localhost:8080/images/${newName}`
        user[data[0]] = imgPath
