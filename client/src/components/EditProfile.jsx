...
function App({ setEditMode }) {
    ...
    const sendPhoto = () => {
        if (loading) return
        if (description.length < 3 || name.length < 3) return toast.error("Please Fill All Fields", {
        ...
        if (banner || photo) {
            ...
            if (banner) {
                ...
                formData.append("upload_preset", "rstsm9cs")

                const imageUpload = async () => {
                    const response = await fetch("https://api.cloudinary.com/v1_1/dwngj1n6n/image/upload", {
                        method: "POST",
                        body: formData
                    });
                    ...
                    try {
                        ...
                        var resp = await axios.post("/update", {
                            id: user.id,
                            description,
                            name,
                            banner: res.url,
